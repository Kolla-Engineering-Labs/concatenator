...
test.describe.serial('Concatenate Mode', () => {
  ...
  test.describe('File Upload via Drag and Drop', () => {
    test('should accept directory drag and drop', async ({ page }) => {
      ...
      try {
        ...
        const files = [
          { name: 'readme.md', path: 'project/readme.md', content: '# Test Project' },
          { name: 'main.js', path: 'project/src/main.js', content: 'console.log("hello");' },
          { name: 'utils.js', path: 'project/src/utils.js', content: 'export const sum = (a,b) => a+b;' },
        ];
        ...
        // Verify file count
        await expect(page.getByText(/Selected Files.*3/)).toBeVisible({ timeout: 10000 });
      } finally {
    ...
    test('should handle nested directory structure', async ({ page }) => {
      ...
      try {
        const files = [
          { name: 'package.json', path: 'my-app/package.json', content: '{"name": "test"}' },
          { name: 'App.tsx', path: 'my-app/src/App.tsx', content: 'export default () => <div />;' },
          { name: 'index.css', path: 'my-app/src/styles/index.css', content: 'body { margin: 0; }' },
          { name: 'Button.tsx', path: 'my-app/src/components/Button.tsx', content: 'export const Button = () => {};' },
        ];
        ...
        // Should show 4 files
        await expect(page.getByText(/Selected Files.*4/)).toBeVisible({ timeout: 10000 });
      } finally {
        uploadHelper.cleanup();
      }
    });
    ...
    test('should show processing state during upload', async ({ page }) => {
      ...
      try {
        ...
        const progressLocator = page.locator('span').filter({ hasText: /Reading Files\.\.\.|Scanning Folder\.\.\./ });
        ...
        await expect(page.getByText('file0.txt')).toBeVisible({ timeout: 15000 });
    ...
    test('should add ignore patterns', async ({ page, browserName }) => {
      ...
      try {
        ...
        const files = [
          ...
          { name: 'app.test.js', path: 'src/app.test.js', content: 'test("app")' },
  ...
  test.describe('Settings Modal', () => {
    test('should open and close settings modal', async ({ page }) => {
      // Click settings button using JavaScript click for Firefox compatibility
    ...
    test('should hold API keys in memory for the session only', async ({ page }) => {
      ...
      // Fill in API keys
      await page.locator('#gemini-api-key').fill('test-gemini-key-123');
      await page.locator('#openai-api-key').fill('test-openai-key-456');
      await page.locator('#anthropic-api-key').fill('test-anthropic-key-789');

      // Wait for any animations to complete before saving
      await page.waitForTimeout(500);
    ...
    test('should NOT load API keys from localStorage on page load', async ({ page }) => {
      ...
      // Wait for modal to be visible (animation takes time with framer-motion)
      const modal = page.locator('div').filter({ hasText: /^Settings$/ }).first();
      await expect(modal).toBeVisible({ timeout: 5000 });

      // Wait for modal animation to complete before accessing inputs
      await page.waitForTimeout(300);

      // Wait for inputs to be visible before asserting values
      const geminiInput = page.locator('#gemini-api-key');
      const openaiInput = page.locator('#openai-api-key');
      const anthropicInput = page.locator('#anthropic-api-key');
    ...
    test('should switch between list and tree view', async ({ page }) => {
      ...
      try {
        ...
        const files = [
          ...
          { name: 'helper.js', path: 'project/src/helper.js', content: 'helper' },
        ];
        await uploadHelper.setFilesOnInput(files);
        ...
        await expect(treeButton).toHaveClass(/bg-white|shadow-sm/, { timeout: 10000 });
        ...
        await expect(page.getByText('utils.js')).toBeVisible({ timeout: 10000 });
        await expect(page.getByText('helper.js')).toBeVisible({ timeout: 10000 });
      } finally {
        uploadHelper.cleanup();
      }
    });
  ...
  test.describe('File Actions', () => {
    test('should ignore individual files from the list', async ({ page }) => {
      ...
      try {
        ...
        await expect(page.getByText(keepFileName)).toBeVisible({ timeout: 10000 });
        await expect(page.getByText(ignoreFileName)).toBeVisible({ timeout: 10000 });

        // Find and click the ignore button for the ignore file using title attribute
        const ignoreButton = page.locator(`button[title="Ignore ${ignoreFileName}"]`);
        ...
        await expect(fileListContainer.getByText(ignoreFileName, { exact: false })).toHaveCount(0, { timeout: 10000 });

        // Verify keep file is still visible (as a file row with buttons)
        const keepFileRow = page.locator('button[title="Remove ' + keepFileName + '"]').first();
    ...
    test('should remove files from selection', async ({ page }) => {
      ...
      try {
        ...
        await expect(page.getByText('file1.js')).not.toBeVisible({ timeout: 10000 });

        // Then check the file count updated to 1
        await expect(page.getByText(/Selected Files.*\(1\)/)).toBeVisible({ timeout: 10000 });
  ...
  test.describe('Concatenate & Download', () => {
    test('should enable concatenate button when files are present', async ({ page }) => {
      const uploadHelper = new FileUploadHelper(page);

      try {
        // Initially disabled
        const concatButton = page.getByRole('button', { name: /Concatenate & Download/ });
        await expect(concatButton).toBeDisabled();

        // Upload files
        const files = [{ name: 'script.js', path: 'script.js', content: 'alert("hi")' }];
        await uploadHelper.setFilesOnInput(files);
    ...
    test('should download concatenated file', async ({ page }) => {
      ...
      try {
        const files = [
          ...
          { name: 'goodbye.js', path: 'src/goodbye.js', content: 'const goodbye = "moon";' },
        ];
        await uploadHelper.setFilesOnInput(files);
