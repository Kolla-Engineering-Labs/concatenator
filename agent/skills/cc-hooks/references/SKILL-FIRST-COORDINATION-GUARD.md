...
_and_ the doctrine. The skill knows the reservation protocol, the inbox model,
...
**over-matches**: `grep` is line-oriented, so `^` matches _every_ heredoc-body
...
would falsely fire even though no coordination command is being _run_.
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "~/.claude/hooks/skill-first-coord-guard.sh"
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "~/.claude/hooks/skill-first-coord-mark.sh"
          }
        ]
      }
    ]
...
This is by design: the guard is an opt-in _nudge_, not a security boundary. The
