...
_before_ the measurement is run, so the success criterion and the null-tolerance
...
> accrue. So ADR-0002 l.58 is _not cleared at landing_ — it becomes clearable once
...
telemetry it consumes _is_ that evidence pipeline.
...
```json
{
  ...
  "session": "<session_id>",
  "token_class": "installed-skill-edit",
  "path_sha256": "<64-hex>"
}
```
...
The hash is one-way; it lets us count _distinct_ edited targets and detect
...
**Gate-blind:** the sensor records the _attempt_, not the outcome of the
...
skills around it) stops _attempting_ the mistake. That is a learning signal that
the gate's own redirect **cannot fabricate** — the redirect fires _after_ the
...
- The gate's redirect lowers the post-redirect hand-roll rate _by construction_ —
...
The attempt rate over time sidesteps all three: it is measured on the _input_
side of the redirect, so the redirect cannot move it; the trend is across the
agent's _own_ history, needing no off-arm counterfactual.
...
- **NULL is ACCEPTABLE (KEEP-on-no-harm):** if at N ≥ 30 the rate is flat or the
  ...
  satisfies the ADR-0002 l.58 "lease on life" as _no harm + a measurable signal
  channel that exists and runs_. A guard that is provably silent and provably
