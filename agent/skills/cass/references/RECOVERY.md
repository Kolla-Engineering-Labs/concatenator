...
**Diagnosis:** The index _data_ committed successfully. Only `persist_final_index_run_metadata` (src/indexer/mod.rs:6295) lost the writer race against a concurrent cass process. `cass status` keeps reporting "stale" because the freshness marker never landed.
