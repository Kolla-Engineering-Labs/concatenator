...
For per-model spend, use `cass analytics models --json` (next section); `tokens` does not break down by model. For Claude Max / GPT Pro flat-rate accounts, the dollar number is irrelevant — what matters is _throughput per account_ (use `caam` to plan account allocation).
