...
> Alignment trains models toward the _mode_ of human preference, so prose goes
...
> prefab _and_ checks that something specific is still present (the additive
> floor below).
...
Flag the _mechanical append_ — a clause fused with a dash where a comma, colon,
