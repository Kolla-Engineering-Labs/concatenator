...
This file is the canonical guide for: (1) how to _see_ a fail-open, (2) what each fail-open reason means, and (3) what to do about it before asking the human.
...
Two more variants — `affinity_pinned` and `affinity_fallback` — are _success_ paths (a worker was assigned via affinity), not fail-opens, so they never appear in `[RCH] local (...)` output.
