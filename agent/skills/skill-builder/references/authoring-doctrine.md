...
_process_ on every run. Structure (frontmatter, sections, output contracts)
...
A fuzzy exit invites _premature completion_: attention drifts from the work
...
region: _tight_ (loop), _red_ (failing check), _fresh_ (context),
_frontier_, _quarantine_. Used as a token — not re-explained each time —
...
"fast and deterministic" are one idea; _tight_ says it once. Invented terms
