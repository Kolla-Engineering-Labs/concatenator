...
export const FileTable: React.FC<FileTableProps> = ({
  ...
  const sortedFiles = useMemo(() => {
    return [...files].sort((a, b) => {
      let valA: string | number = a[sortField as keyof FileItem] as
        | string
        | number
      let valB: string | number = b[sortField as keyof FileItem] as
        | string
        | number

      if (sortField === 'tokens') {
        valA = estimateTokenCount(a.content, a.size)
        valB = estimateTokenCount(b.content, b.size)
  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full text-left border-collapse">
        <thead>
          <tr className="border-b border-slate-100 dark:border-slate-800 text-[10px] uppercase tracking-wider text-slate-500 font-bold bg-slate-50/50 dark:bg-slate-900/50">
            <th
        ...
        <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
          {sortedFiles.map((file, idx) => (
            ...
            >
              ...
              <td className="px-4 py-2.5 text-right font-mono text-[10px] text-brand-500">
                {estimateTokenCount(file.content, file.size).toLocaleString()}
              </td>
              <td className="px-4 py-2.5 text-right">
                <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  ...
                  <button
                    onClick={() => {
                      const pattern = file.kind === 'directory' ? `${file.path}/**` : file.path
                      if (file.isIgnored) {
